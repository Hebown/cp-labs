...
编译方法和之前一致。这里我就不再强调环境要求了。不过我现在构建的类型是 release，所以你可能需要将链接时候的opencv库改为release版本的（之前我也没有强调这一点，以及你应该链接正确的opencv库）

我直接给出按照我上传的结构：
- demo: 内部提供了我打包的二进制结果，之后会告诉你如何运行
- report: 内部提供了我的实验报告
- src: 内部提供了我的源码，包含c++文件和cmake依赖文件

如果你想编译，请进入src，然后修改其中关键的第三方库依赖，之后运行编译（别忘了vcvars64.bat）
如果你想运行demo，请进入 demo，在里面我已经提供了完整的（我本地编译出来的结果）。如果你想得到我的实验一致的图片拼接结果，那么你只需要在demo的根目录下运行我准备好的ps1脚本即可。随机你就会得到所有的处理后的图像（默认位于 lab_result目录下）。防止你搞不出来（很小可能），我放了一组expected供你参考
